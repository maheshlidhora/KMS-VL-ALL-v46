...
if %winbuild% GEQ 9200 (exit /b)
if %winbuild% LSS 9200 (if %loc_off14% EQU 0 (exit /b))
)
set sub_O365=0
set sub_proj=0
set sub_vis=0
...
%_qr% %_Nul2% | findstr /i "%app%" %_Nul1% && (call :activate&exit /b)
...
%_qr% %_Nul2% | findstr "1" %_Nul1% && (call :activate&exit /b)
set "_qr=%_zz1% %spp% %_zz2% %_zz5%PartialProductKey is not NULL%_zz6% %_zz3% ID %_zz4%"
%_qr% %_Nul2% | findstr /i "%app%" %_Nul1% && (call :activate&exit /b)
...
rem echo.
rem echo Microsoft Office is activated with a subscription.
)
if %sub_proj% EQU 1 (
